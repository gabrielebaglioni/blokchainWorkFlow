...
2. Configura le variabili d'ambiente:
   ```bash
   cd services/content-layer/ai-content-engine-ms
   cp .env.example .env
...
pnpm --filter ai-content-engine-ms dev
```

Il servizio sarà disponibile su `http://localhost:3007
6379`
...
docker build -f Dockerfile.dev -t houseblock/ai-content-engine-ms-dev .

# Avvio container
docker run --env-file .env.local -p 3007
6379:3007
6379 houseblock/ai-content-engine-ms-dev
```
...
docker build -t houseblock/ai-content-engine-ms .

# Avvio container
docker run --env-file .env.production -p 3007
6379:3007
6379 houseblock/ai-content-engine-ms
```
