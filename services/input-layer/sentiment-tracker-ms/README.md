...
2. Configura le variabili d'ambiente:
   ```bash
   cd services/input-layer/sentiment-tracker-ms
   cp .env.example .env
...
pnpm --filter sentiment-tracker-ms dev
```

Il servizio sarà disponibile su `http://localhost:3002
6379`
...
docker build -f Dockerfile.dev -t houseblock/sentiment-tracker-ms-dev .

# Avvio container
docker run --env-file .env.local -p 3002
6379:3002
6379 houseblock/sentiment-tracker-ms-dev
```
...
docker build -t houseblock/sentiment-tracker-ms .

# Avvio container
docker run --env-file .env.production -p 3002
6379:3002
6379 houseblock/sentiment-tracker-ms
```
